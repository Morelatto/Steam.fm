...
jQuery( function($) {
	...
	$('#responsive_beta_logo' ).click( function() {
		...
		var $ReportBugButton = $J('<div/>', {'class': ' btn_darkblue_white_innerfade btn_medium'} ).append( $J('<span/>').text( 'Relatar um bug' ) );

		var $Description = $J('<div/>', {style: 'font-size: 1.2em;' } );
		$Description.append( $J('<h2/>', {style: "color: #ffffff; margin-top: 12px; margin-left: 12px;"} ).text('Bem-vindo(a)!') );
		$Description.append( $J('<p/>', {style: "margin-left: 12px;"}).append( "Estamos testando uma nova atualiza\u00e7\u00e3o aos sites do Steam que usa design responsivo para uma melhor experi\u00eancia em celulares, tables, laptops com telas t\u00e1teis e outros dispositivos.<br \/>\r\nVisite o f\u00f3rum do Steam M\u00f3vel para saber mais ou deixar feedback." ) );

		//$Description.append( $J('<p/>').append( "Voc\u00ea tamb\u00e9m pode relatar problemas enfrentados com as novas p\u00e1ginas aqui:" ) );
		$Description.append( $J('<div/>', {'class': 'newmodal_buttons', 'style': 'text-align: left; margin-bottom: 8px;'} ).append(
			$J('<a/>', {'href': 'http://steamcommunity.com/discussions/forum/8/', 'class': ' btn_darkblue_white_innerfade btn_medium'} ).append( $J('<span/>').text( 'Visitar fórum do Steam Móvel' ) )
		).append( $ReportBugButton ) );

		ShowDialog(
			'Steam web responsivo — Beta',
			$Description
		);
		...
		$ReportBugButton.click( function() {
			...
			var strPost = '[quote]\n' + rgSpecs.join( '\n' ) + '\n[/quote]\n\nPreencha o seu relatório de bug aqui:\n\n';

			PostToURLWithSession( 'http://steamcommunity.com/discussions/forum/8/537405286642983692/', {
				discussions_reply_prefill: strPost,
				discussions_reply_prefill_prompt: 'Já preenchemos algumas informações sobre o seu dispositivo para nos ajudar a diagnosticar o problema. Por favor, adicione os seus detalhes no fim da publicação.'
			} );
		});
	});
		...
		$SelectBox.append(
			$J('<option/>', {value: "brazilian"} ).text( 'Português (Brasil)' )
				...
				$J('<option/>', {value: "schinese"} ).text( '简体中文 (Chinês Simplificado)' )
					...
					$SelectBox.append(
				$J('<option/>', {value: "spanish"} ).text( 'Español (Espanhol)' )
			);
					$SelectBox.append(
