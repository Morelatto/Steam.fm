		...
		switch ( responseJSON.success )
		{
			...
			case 24:
				ShowAlertDialog( 'Erro',
					'A sua conta não atende aos requisitos para usar este recurso. <a class="whiteLink" href="https://support.steampowered.com/kb_article.php?ref=3330-IAGK-7663" target="_blank">Acesse o Suporte Steam</a> para mais informações.'
				);
				break;
			case 15:
				ShowAlertDialog( 'Erro', 'Você não tem permissão para realizar essa ação.' );
				break;
			default:
				ShowAlertDialog( 'Erro', 'Ocorreu um erro ao processar a sua solicitação:' + responseJSON.success );
				break;
		}
	} );
	return false;
...
CAjaxInfiniteScrollingControls.prototype.RestoreScrollTop = function( bForce )
{
	...
	if ( scrollTopPrevious != 0 )
	{
		...
		else
		{
			...
			{
				this.m_LoadingDialog = ShowBlockingWaitDialog( 'Por favor, aguarde', 'Retornando para onde você estava nesta página pela última vez...' );
			}
			window.scrollTo( 0, scrollTopPrevious );
		}
	}
