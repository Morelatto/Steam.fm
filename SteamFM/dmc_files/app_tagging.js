...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Você aplicou este marcador. Clique para removê-lo.' );
		else
			$Checkbox.data('store-tooltip', 'Clique aqui para aplicar este marcador.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Você denunciou este marcador por ter sido aplicado a este produto incorretamente. Clique aqui para remover a denúncia.' );
		else
			$ReportFlag.data('store-tooltip', 'Clique para denunciar este marcador por ter sido aplicado a este produto incorretamente.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Clique aqui para remover este marcador do produto. Esta opção é restrita a desenvolvedores e funcionários do Steam.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Banir marcador', 'Tem certeza de que deseja remover o marcador "%s" deste produto?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Ainda não há itens suficientes marcados com "%s"'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Clique aqui para aplicar este marcador.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/brazilian').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Ainda não há itens suficientes marcados com "%s"'.replace( /%s/, $AppTag.html() ) );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'Lamentamos, houve um problema ao carregar itens. Por favor, tente novamente mais tarde.' );
			});
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Remover banimento',
				'Tem certeza de que deseja remover o banimento no marcador "%s"?'.replace( /%s/, tag ),
				'Remover banimento')
				.done( function() {
					var rgParams = {
						appid: appid,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Remover banimento', 'Houve um problema ao banir ou remover o banimento deste marcador. Por favor, tente novamente mais tarde.' );
					});
					bBansChanged = true;
				});
		});
